...
- **New: `binary_sensor` Quick Charge** — on while a quick charge boost is
  running (hold 233 bit0). Tracks boosts started from anywhere: HA, the
...
- **Changed: Quick Charge Duration select** — the "0" option and the hidden
  enable piggyback are gone (legacy design; newer firmware rejects a 0 write to
  register 234 with Illegal Data Value). The select now only sets the duration;
  starting is the switch's job. Also now correctly hidden on legacy/AC-coupled
  units that don't have the registers.
